...
export default function ProducaoCompletaPage() {
...
function Conteudo({ setFicha, setEtiqueta }) {
  ...
  const [modalEtq, setModalEtq] = useState(null);
  ...
  async function carregar() {
    ...
    const [r1, r2, r3, r4, r5, r6, r7, r8] = await Promise.all([
      ...
      supabase.from('etiqueta_impressoes').select('source_id').eq('empresa_id', eid).eq('source_type', 'producao'),
    ]);
    setLista(r1.data || []);
    setProdutos(r2.data || []);
  ...
  return (
    <>
      ...
      {modalEtq && (
        <ModalEtiquetas
          sourceType="producao"
          tipo={impressoes.some(i => i.source_id === modalEtq.id) ? 'reimpressao' : 'original'}
          producao={{
            id: modalEtq.id,
            codigo: modalEtq.lote,
            produtoNome: modalEtq.produtos?.nome || '—',
            produtoCodigo: modalEtq.produtos?.codigo,
            produzido_em: modalEtq.data,
            validade: modalEtq.validade,
            conservacao: null,
            recipientes: Math.max(1, Math.round(Number(modalEtq.quantidade) || 1)),
            modelo: 'validade-cozinha',
          }}
          empresaNome={empresaAtual?.nome}
          responsavelNome={modalEtq.funcionarios?.nome || '—'}
          onFechar={() => { setModalEtq(null); carregar(); }}
