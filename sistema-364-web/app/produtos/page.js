...
const MP_VAZIA = { nome: '', unidade: 'kg', custo_unitario: '' };
...
function Conteudo() {
  ...
  async function addMP(e) {
    e.preventDefault();
    const { error } = await supabase.from('materias_primas').insert([{ ...formMP, custo_unitario: Number(formMP.custo_unitario), empresa_id: empresaAtual.id }]);
    ...
    <>
      <div className="panel">
        ...
        <div className="table-wrap" style={{ marginTop: 14 }}>
          <table>
            <thead><tr><th>Nome</th><th>Unidade</th><th>Custo padrão</th><th></th></tr></thead>
            <tbody>
              ...
              )) : <tr className="empty-row"><td colSpan={4}>Nenhuma matéria-prima.</td></tr>}
            </tbody>
          </table>
        </div>
      </div>
