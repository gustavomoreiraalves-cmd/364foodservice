...
// Barras por dia empilhadas em três: mesa embaixo, balcão/outros no meio,
// delivery em cima. Mesmo estilo do SerieMensal: SVG próprio, sem biblioteca.
export default function SerieDiariaPdv({ dados }) {
  ...
  return (
    <>
      <svg viewBox={`0 0 ${W} ${H}`} width="100%" role="img" aria-label="Vendas por dia, mesa, balcão/outros e delivery">
        <line x1={L} y1={base} x2={W - R} y2={base} stroke="var(--border)" strokeWidth="1" />
        {dados.map((d, i) => {
          const x = L + faixa * (i + 0.5) - barra / 2;
          const topoMesa = y(d.mesa);
          const topoOutro = y(d.mesa + d.outro);
          const topoTotal = y(d.total);
          return (
            <g key={d.dia}>
              <rect x={x} y={topoMesa} width={barra} height={Math.max(base - topoMesa, 0)} fill="var(--amber)" />
              <rect x={x} y={topoOutro} width={barra} height={Math.max(topoMesa - topoOutro, 0)} fill="var(--paper-dim)" />
              <rect x={x} y={topoTotal} width={barra} height={Math.max(topoOutro - topoTotal, 0)} fill="var(--smoke)" />
              {mostrarRotulo(i) && (
      ...
      <div style={{ display: 'flex', gap: 16, fontSize: 11.5, color: 'var(--paper-dim)', marginTop: 6 }}>
        ...
        <span><b style={{ color: 'var(--paper-dim)' }}>▬</b> Balcão/outros</span>
